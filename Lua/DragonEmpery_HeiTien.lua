...
local modifier_1 = 'HEI_TIEN_COMBAT_BUFF'
local modifier_2 = 'HEI_TIEN_COMBAT_ATTACH'
local percent_1 = 0.2
...
--GreatWork Buff
function HeiTienOnGreatWorkCreated(playerID, unitID, iCityPlotX, iCityPlotY, buildingID, greatWorkIndex)
    --is Hei Tien?
    if not DragonEmperyLeaderTypeMatched(playerID, 'LEADER_HEI_TIEN') then
        return
    end
...
--On tech completed
function HeiTienOnResearchCompleted(iPlayer, iTech)
    --is hei tien?
    if DragonEmperyLeaderTypeMatched(iPlayer, 'LEADER_HEI_TIEN') then
        --get the player
...
function HeiTienActivateGreatWriter(playerID, param)
    --get the player
    local pPlayer = Players[playerID]
    ...
    if playerCulture then
        ...
        --the float string
        local fstring = Locale.Lookup('LOC_HEI_TIEN_GREAT_WRITER_CULTURE', culture)
        --add the view
        local fstringData = {
            MessageType = 0,
            MessageText = fstring,
    ...
    if param.cityID then
        ...
        --the float string
        local fstring = Locale.Lookup('LOC_HEI_TIEN_GREAT_WRITER_PRODUCTION', production)
        --add the view
        local fstringData = {
            MessageType = 0,
            MessageText = fstring,
...
function Initialize()
    -----------------Events-----------------
    Events.GreatWorkCreated.Add(HeiTienOnGreatWorkCreated)
    Events.ResearchCompleted.Add(HeiTienOnResearchCompleted)
    ---------------GameEvents---------------
    GameEvents.HeiTienGreatWriterActivated.Add(HeiTienActivateGreatWriter)
    ----------------------------------------
    print('DragonEmpery_HeiTien Initial success!')
end
